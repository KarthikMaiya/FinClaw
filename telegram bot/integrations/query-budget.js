...
const { getActualBudgetConfig, validateRequiredConfig, printDebugSummary } = require('../config-manager');
...
const BUDGET_LOCAL_ID = 'My-Finances-7655c8a';
  ...
  if (!apiInitPromise) {
    apiInitPromise = (async () => {
      try {
        ...
        console.log('[query-budget] Budget API initialized successfully');
      } catch (error) {
        initFailureCount += 1;
